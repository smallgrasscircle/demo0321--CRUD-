...
<html>
    ...
    %>
			...
			<s:form action="/stuaction!update" method="post" name="ddd" id="123">
			    <s:hidden name="stu.sid" value="%{#session.updatestu.sid}"></s:hidden>
				<s:textfield name="stu.sname" label="学生姓名" value="%{#session.updatestu.sname}"></s:textfield>
				<s:textfield name="stu.sno" label="学生学号" value="%{#session.updatestu.sno}"></s:textfield>
				<s:radio list="{'男','女'}" name="stu.sex" label="性别" value="%{#session.updatestu.sex}"></s:radio>
				<s:password name="stu.password" label="密码" value="%{#session.updatestu.password}"></s:password>
				<br />  系部与班级要级联 <br />
                ...
                <s:select list="{'江苏','浙江','安徽','上海','山东','福建'}" label="所在省份" name="stu.shengfen" value="%{#session.updatestu.shengfen}"></s:select>
		        ...
		        <s:textarea label="其他信息" name="stu.beizhu" cols="30" rows="5" value="%{#session.updatestu.beizhu}"></s:textarea>
		        <td align="right"><s:submit value="修改" theme="simple"></s:submit></td>
